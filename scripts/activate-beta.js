...
async function main() {
  ...
  if (!arg) throw new Error('Usage: node scripts/activate-beta.js <UID_OR_EMAIL>')
  ...
  // Set betaActive flag in Firestore profile
  await admin.firestore().collection('profiles').doc(uid).set({ betaActive: true }, { merge: true })

  console.log(`✅ activated beta access for ${uid}${user.email ? ` (${user.email})` : ''}`)
}
